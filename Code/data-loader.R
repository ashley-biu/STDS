...
load_employment_data <- function(research_data) {
  ...
  nsw_employment$month <- month(nsw_employment$date)
  
  # create new employment year column for year from date
  nsw_employment$year <- year(nsw_employment$date)
  ...
  research_data <- merge(research_data, nsw_emp_quarterly, by = "time_frame")
  return(research_data)
}
...
load_NSW_population_data <- function(research_data){
  ...
  research_data <- merge(research_data, nsw_youth_prop_quaterly, by="time_frame")
...
load_registration_data <- function(research_data) {
  ...
  nsw_vehicles_registered$month <-
    month(nsw_vehicles_registered$QUARTER)
  
  # create new CPI column for year from date
  nsw_vehicles_registered$year <-
    year(nsw_vehicles_registered$QUARTER)
  
  nsw_vehicles_registered <- nsw_vehicles_registered %>% mutate(time_frame = case_when(
    between(month, 1, 3) ~ paste(year,"-Q1", sep = ""),
    between(month, 4, 6) ~ paste(year,"-Q2", sep = ""),
    between(month, 7, 9) ~ paste(year,"-Q3", sep = ""),
    between(month, 10, 12) ~ paste(year,"-Q4", sep = ""),
  ...
  registerd_vehicles_quarterly <- nsw_vehicles_registered %>%
    group_by(time_frame) %>%
  ...
  research_data <- merge(research_data, registerd_vehicles_quarterly, by = "time_frame")
